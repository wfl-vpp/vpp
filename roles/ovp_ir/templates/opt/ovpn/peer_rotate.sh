...
SERVICE_FILE="/etc/systemd/system/openvpn-irclient.service"

# Port to check for reachability
PORT={{ fs_server_port }}

# Read the current IP from the service file
CURRENT_IP=$(grep -oE -- '--remote \S+' "$SERVICE_FILE" | awk '{print $2}')
...
for ip in "${IPS[@]}"; do
  ...
  if is_port_open "$ip"; then
    ...
    sed -i "s|--remote $CURRENT_IP|--remote $ip|" "$SERVICE_FILE"
